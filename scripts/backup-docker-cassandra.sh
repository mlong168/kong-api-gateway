...
docker run --rm --volumes-from cassandra-data -v $(pwd):/backup mini/base sh -c "tar -czf /backup/$tarball_file $datadir/*/snapshots/$name"

# assume /var/lib/cassandra is the volume in the container (for now)
if [ $? -eq 0 ] ; then
        aws s3 cp $tarball_file "${bucket}/${tarball_file}"
fi
